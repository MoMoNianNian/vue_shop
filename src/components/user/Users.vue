  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="userList" border stripe>
        ...
        <el-table-column label="电话" prop="mobie"></el-table-column>
...
<script>
import { getUserList, getUsers, getEditUser, getSubmitUser } from "../../api/login";
  ...
  methods: {
    ...
    adduser() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (!valid) return;
        // 可以发起添加用户的网络请求
        const { data: res } = await getUsers(this.addForm);
    ...
    editUserInfo(){
      this.$refs.editFormRef.validate(async (valid) => {
        if (!valid) return;
        // 可以发起添加用户的网络请求
        const { data: res } = await getSubmitUser({
    ...
    async removeUserById(id){
      ...
      const comfirmResult = await this.$confirm('此操作将永久删除该用户, 是否继续?', '提示', {
        ...
        // 如客户确认删除，返回的是字符串confirm
        // 如客户取消删除，返回的是字符串cancel
